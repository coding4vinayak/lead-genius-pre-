...
export function Card({ children, className = '', ...props }: { children: ReactNode; className?: string; [key: string]: unknown }) {
  return <div className={`bg-[var(--color-surface)] rounded-xl border border-[var(--color-border)] shadow-sm transition-all duration-200 hover:-translate-y-0.5 hover:shadow-lg ${className}`} {...props}>{children}</div>;
}
...
export function Input({ label, error, ...props }: { label?: string; error?: string; [key: string]: unknown }) {
  return (
    <div className="space-y-1">
      ...
      <input className={`w-full px-3 py-2 border rounded-lg text-sm bg-[var(--color-surface)] text-[var(--color-text)] focus:outline-none focus:ring-2 focus:ring-[var(--color-primary)] focus:border-transparent transition-all duration-200 ${error ? 'border-red-400' : 'border-[var(--color-border)]'}`} {...props} />
...
export function Select({ label, options, error, ...props }: { label?: string; options: Array<string | { value: string; label: string }>; error?: string; [key: string]: unknown }) {
  return (
    <div className="space-y-1">
      {label && <label className="block text-sm font-medium text-[var(--color-text-secondary)]">{label}</label>}
      <select className={`w-full px-3 py-2 border rounded-lg text-sm bg-[var(--color-surface)] text-[var(--color-text)] focus:outline-none focus:ring-2 focus:ring-[var(--color-primary)] focus:border-transparent transition-all duration-200 ${error ? 'border-red-400' : 'border-[var(--color-border)]'}`} {...props}>
        {options.map((opt) => (
...
export function Modal({ isOpen, onClose, title, children }: { isOpen: boolean; onClose: () => void; title: string; children: ReactNode }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-[var(--color-overlay)] backdrop-blur-sm" onClick={onClose}>
      <div className="bg-[var(--color-surface)] rounded-xl shadow-xl w-full max-w-lg mx-4 max-h-[85vh] overflow-y-auto animate-in fade-in duration-200" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between p-4 border-b border-[var(--color-border)]">
          <h2 className="text-lg font-semibold text-[var(--color-text)]">{title}</h2>
          <button onClick={onClose} className="p-1 hover:bg-[var(--color-surface-tertiary)] rounded-lg transition-colors text-[var(--color-text-secondary)]">&times;</button>
        </div>
...
export function EmptyState({ title, description, action }: { title: string; description: string; action?: ReactNode }) {
  return (
    <div className="flex flex-col items-center justify-center py-16 text-center">
      <div className="w-16 h-16 bg-[var(--color-surface-tertiary)] rounded-full flex items-center justify-center mb-4">
    ...
    <div className="flex border-b border-[var(--color-border)] mb-4">
      {tabs.map((tab) => (
        ...
        >
          {tab.label}
          {tab.count !== undefined && (
            <span className="ml-2 bg-[var(--color-surface-tertiary)] text-[var(--color-text-secondary)] rounded-full px-2 py-0.5 text-xs">{tab.count}</span>
          )}
        </button>
      ))}
    </div>
...
export function ProgressBar({ value, max = 100, className = '' }: { value: number; max?: number; className?: string }) {
  ...
  return (
    <div className={`w-full bg-[var(--color-surface-tertiary)] rounded-full h-2 overflow-hidden ${className}`}>
      <div
...
export function StatCard({ icon, label, value, trend }: { icon: ReactNode; label: string; value: string | number | ReactNode; trend?: { value: number; positive: boolean } }) {
  return (
    <Card className="p-4">
...
export function DropdownMenu({ trigger, items }: { trigger: ReactNode; items: { label: string; onClick: () => void; icon?: ReactNode; danger?: boolean }[] }) {
  ...
  return (
    <div className="relative" ref={ref}>
      ...
      {open && (
        <div className="absolute right-0 mt-1 w-48 bg-[var(--color-surface)] rounded-lg shadow-lg border border-[var(--color-border)] py-1 z-50">
          {items.map((item) => (
            <button
              key={item.label}
              onClick={() => { item.onClick(); setOpen(false); }}
              className={`w-full text-left px-3 py-2 text-sm flex items-center gap-2 hover:bg-[var(--color-surface-tertiary)] transition-colors ${item.danger ? 'text-red-600' : 'text-[var(--color-text-secondary)]'}`}
            >
              {item.icon}
              {item.label}
            </button>
