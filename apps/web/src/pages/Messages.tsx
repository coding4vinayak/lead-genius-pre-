            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-[var(--color-border)] bg-[var(--color-surface-secondary)]">
                  <th className="p-3 text-left font-medium text-[var(--color-text-secondary)] text-xs uppercase tracking-wider">Lead</th>
                  <th className="p-3 text-left font-medium text-[var(--color-text-secondary)] text-xs uppercase tracking-wider">Channel</th>
                  <th className="p-3 text-left font-medium text-[var(--color-text-secondary)] text-xs uppercase tracking-wider">Status</th>
                  <th className="p-3 text-left font-medium text-[var(--color-text-secondary)] text-xs uppercase tracking-wider">Campaign</th>
                  <th className="p-3 text-left font-medium text-[var(--color-text-secondary)] text-xs uppercase tracking-wider">Sent</th>
                </tr>
              </thead>
              <tbody>
                {messages.map((msg: any) => (
                  <tr key={msg.id} className="border-b border-[var(--color-border-light)] hover:bg-[var(--color-surface-secondary)] transition-colors">
                    <td className="p-3 text-[var(--color-text)]">{msg.lead?.name || msg.lead?.email || '—'}</td>
                    <td className="p-3">{msg.channel === 'email' ? <Mail size={14} className="text-[var(--color-info)]" /> : <MessageCircle size={14} className="text-[var(--color-success)]" />}</td>
                    <td className="p-3"><Badge variant={STATUS_COLORS[msg.status] || 'default'}>{msg.status}</Badge></td>
                    <td className="p-3 text-[var(--color-text-secondary)]">{msg.campaign?.name || '—'}</td>
                    <td className="p-3 text-[var(--color-text-tertiary)] text-xs">{new Date(msg.createdAt).toLocaleString()}</td>
                  </tr>
                ))}
              </tbody>
            </table>
